...
private:
    ...
    bool FreeSpace(size_t size);
    bool DeleteOldest();
    bool DeleteIt(std::map<std::reference_wrapper<const std::string>,
                                      std::reference_wrapper<lru_node>,
                                      std::less<std::string>>::iterator it);
    bool UpdatePtr(lru_node *node, const std::string &value);
    void Move2Tail(lru_node *node);
};
