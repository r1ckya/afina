...
void ServerImpl::OnRun() {
    ...
    while (running.load()) {
...

        {
            if (workers_count < max_workers) {
                ...
                std::thread(&ServerImpl::Handler, this, client_socket).detach();
                ++workers_count;
            } else {
                _logger->warn("No free workers, closing connection {}\n", client_socket);
                close(client_socket);
            }
        }
    ...
    while (workers_count.load()) {
        check_workers.wait(lock);
    }
    _logger->warn("Network stopped");
}

void ServerImpl::Handler(int client_socket) {
    // Here is connection state
    // - parser: parse state of the stream
    // - command_to_execute: last command parsed out of stream
    // - arg_remains: how many bytes to read from stream to get command argument
